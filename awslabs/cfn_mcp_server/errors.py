...
def handle_aws_api_error(e: Exception, context: str = None) -> Exception:
    ...
    if 'AccessDenied' in error_message or error_type == 'AccessDeniedException':
        ...
        # Generic error handling - include context if provided
        if context:
            return ClientError(f'An error occurred in {context}: {error_message}')
        else:
