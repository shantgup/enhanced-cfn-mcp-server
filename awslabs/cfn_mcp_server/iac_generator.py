...
async def _start_template_generation(
    ...
    if resources:
        ...
        for resource in resources:
            ...
            # Handle different ResourceIdentifier formats
            resource_identifier = resource['ResourceIdentifier']
            
            # If ResourceIdentifier is a string, convert it to the expected format
            if isinstance(resource_identifier, str):
                # For most resources, the identifier is just the resource name/ID
                resource_identifier = {resource['ResourceType']: resource_identifier}
            elif not isinstance(resource_identifier, dict):
                raise ValueError(f"ResourceIdentifier must be a string or dict, got {type(resource_identifier)}")
